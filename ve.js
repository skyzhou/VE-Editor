	...
	v.$extend({
		...
		_initToolbar:function(){
			...
			if (this.config.floatToolbar) {
				this.toolbarElement = this.createToolPanel(v.util.tmpl(toolbarTpl,{plugins:this.plugins,  prefix:'float'}), this.id);
				bindVeCmdEvt(this.toolbarElement);
			}

			if (this.config.fixedToolbar) {
				this.fixedToolbarWrapperEl.innerHTML = (v.util.tmpl(toolbarTpl,{plugins:this.plugins  , prefix:'fixed'}));
...
;(function(util,v){
	...
	util.writeFrame = function(id,parent,dout,callback,style){
		...
		if (ua.isie || navigator.userAgent.indexOf('Trident') > 0) {
...
;(function(v){
	...
	v.$addPlugin({
		command:'image',
		panel:v.util.tmpl('    <div  class="ve-panel ve-uploadimg" style="background-color:#ffffff">            </div>',{}),
		...
		onInit:function(){
			...
			v.util.bindEvt(this.editorElement,"drop",function(evt){
				...
				if(files && /image/.test(files[0].type)){
					that.uploadImage(files[0],action,{
		...
		onInit:function(){
			...
			this.addEvtListener("keydown",function(evt){
				...
				if(evt.keyCode == 9){
					...
					this.setFocusAt(v.$("content_tab"));
					v.$("content_tab").removeAttribute("id");
					return true;
				}
				//退格
